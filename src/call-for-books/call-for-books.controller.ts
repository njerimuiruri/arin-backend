...
function ensureUploadDir(): string {
  const uploadDir = path.join(__dirname, '../../uploads/call-for-books');
...
@Controller('call-for-books')
export class CallForBooksController {
  constructor(private readonly service: CallForBooksService) {}
  ...
  async uploadImage(@UploadedFile() file: Express.Multer.File) {
    ...
    const url = `/uploads/call-for-books/${file.filename}`;
    return { url };
  }
  ...
  async findOne(@Param('id') id: string) {
    ...
    if (!item) {
      throw new BadRequestException('Call for Books item not found');
    }
    return item;
  }
  ...
  async remove(@Param('id') id: string) {
    ...
    return { message: 'Call for Books item deleted successfully' };
  }
}
