...
function ensureUploadDir(): string {
  const uploadDir = path.join(__dirname, '../../uploads/policy-briefs');
...
export class PolicyBriefController {
  constructor(private readonly service: PolicyBriefService) {}
  ...
  async uploadImage(@UploadedFile() file: Express.Multer.File) {
    ...
    const url = `/uploads/policy-briefs/${file.filename}`;
    return { url };
  }
  ...
  async findOne(@Param('id') id: string) {
    const brief = await this.service.findOne(id);
    if (!brief) {
  ...
  async remove(@Param('id') id: string) {
    ...
    return { message: 'Policy brief deleted successfully' };
  }
}